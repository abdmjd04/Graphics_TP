...
void FourPointClosedPolygon(int N, Point *P, Point *Q, float w)
{
	int i;
	for (i = 0; i < N ; i++)
	{
		...
		Q[2 * i + 1].x = - w * P[(((i-1) % N) + N) % N].x + ((1. / 2) + w)* P[((i % N) + N) % N].x + ((1. / 2) + w)* P[(((i+1) % N) + N) % N].x + -w * P[(((i+2) % N) + N) % N].x;
		Q[2 * i + 1].y = - w * P[(((i-1) % N) + N) % N].y + ((1. / 2) + w)* P[((i % N) + N) % N].y + ((1. / 2) + w)* P[(((i+1) % N) + N) % N].y + -w * P[(((i+2) % N) + N) % N].y;
	}
}

 void GeneralizeFourPointClosedPolygon(int N, Point *P, Point *Q, float w)
{
	int i;
	for (i = 0; i < N ; i++)
	{
...
void UniformsplineClosedPolygon(int N, Point *P, Point *Q)
{
	int i;
	for (i = 0; i < N; i++)
	{

		Q[2 * i].x =  (1./8)*P[(((i-1) % N) + N) % N].x  + (6./8)*P[((i % N) + N) % N].x + (1./8)*P[(((i+1) % N) + N) % N].x  ;
		Q[2 * i].y =  (1./8)*P[(((i-1) % N) + N) % N].y  + (6./8)*P[((i % N) + N) % N].y + (1./8)*P[(((i+1) % N) + N) % N].y;

		Q[2 * i + 1].x = (1./2)*P[((i % N) + N) % N].x + (1./2)*P[(((i+1) % N) + N) % N].x;
		Q[2 * i + 1].y =  (1./2)*P[((i % N) + N) % N].y + (1./2)*P[(((i+1) % N) + N) % N].y;
	}
}
...
int main(int argc, char *argv[])
{
	...
	float w,w1;
// for FourPointClosedPolygon
	w = (sqrt(5)+1)/8.0 + (sqrt(5)-1)/16.0;
// for GeneralizeFourPointClosedPolygon
  w1 = (sqrt(5) - 1)/ 8;
...
//	FourPointClosedPolygon(N, P1, P2, w);
//	GeneralizeFourPointClosedPolygon(N,P1,P2,w1);
//	UniformsplineClosedPolygon(N,P1,P2);
